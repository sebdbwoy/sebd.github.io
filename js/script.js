...
function game() {
  ...
  do {
    ...
    while (choix < 1 || choix > 3) {
        consoleLog.innerHTML =
      '<div class="text__block" id="text-block"><p>Erreur : niveau inconnu ! Entrez un nombre entre 1 et 3</p></div>';
      envoyer.addEventListener("click", function () { 
        choix = userValue;
        consoleLog.innerHTML +=
      '<div class="text__block" id="text-block"><p>Enter a value for choix</p></div>'; 
      })
    }
    if (choix = 1) {
      justePrix = Math.floor(Math.random() * 10) + 1;
    ...
    consoleLog.innerHTML +='<div class="text__block" id="text-block"><p>"Niveau " + ${niveau}</p></div>'
    ...
    do {
      ...
      while (prop < 1 || prop > max) {
        window.alert(
          "Erreur : la proposition n'est pas entre les bornes 1 et " + max
        );
        prop = window.prompt("Enter a value for prop");
      }
      i = i + 1;
